...
class OBBs extends OBB {
    ...
    switch (true) {
      default:
        ...
        switch (collidingPoints.length) {
          case 2:
            diff = collidingPoints[0].pC
              .clone()
              .sub(collidingPoints[1].pC)
              .divideScalar(2);
          ...
          case 4:
            ...
            for (let i = 0; i < collidingPoints.length; i++) {
              for (let j = i + 1; j < collidingPoints.length; j++) {
                if (
                  collidingPoints[i].pC.distanceTo(collidingPoints[j].pC) >
                  distance
                ) {
          ...
          default:
            let ray1 = new Ray(
              this.center.clone(),
              obb.center.clone().sub(this.center).normalize()
            );
            let ray2 = new Ray(
              obb.center.clone(),
              this.center.clone().sub(obb.center).normalize()
            );
        ...
        break;
      case l1 == 1 && l2 == 1:
        diff = containsPoints2[0]
          .clone()
          .sub(containsPoints1[0])
          .divideScalar(2);
      ...
      case l1 == 0 && l2 == 1:
        ...
        break;
      case l1 == 0 && l2 == 2:
        diff = containsPoints2[1]
          .clone()
          .sub(containsPoints2[0])
          .divideScalar(2);
        point = containsPoints2[0].clone().add(diff);
  ...
  calcDepth(point, normal, obb) {
    ...
    let helppoint2 = new Vector3()
      .copy(point)
      .addScaledVector(normal.clone(), point.distanceTo(this.center));
